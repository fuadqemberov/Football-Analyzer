...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 *  Bet365BttsOverUnderConsensusAnalyzer — YALNIZ 4 MARKET
 * ═══════════════════════════════════════════════════════════════════════════
 *  {@link Bet365HundredConsensusAnalyzer}-in kopyasıdır. Tək fərqi: təxmin dəsti.
 *  Bu versiya SADƏCƏ Full Time (MS) üzrə bu 4 nəticəni hesablayır və çap edir:
 *      • FT KG VAR   (BTTS Yes)
 *      • FT KG YOX   (BTTS No)
 *      • FT 2.5 ÜST  (Over 2.5)
 *      • FT 2.5 ALT  (Under 2.5)
 *  Başqa heç bir market (tərəf, İY, 2Y, HT/FT, skor, digər A/U xətləri) çıxmır.
 *
 *  Twin (əkiz oyun) tapma məntiqi, havuz qurulması, güvən hesabı və bütün
 *  filtrlər orijinal klas ilə eynidir:
 ...
 *   • Güvəni 96%-dən aşağı olan oyunlar tamamilə ignore olunur.
 *   • 50-dən az yöntem uyğun gələn oyunlar da ignore olunur.
 * ═══════════════════════════════════════════════════════════════════════════
 */
public class Bet365BttsOverUnderConsensusAnalyzer {
    ...
    private static List<Method> buildMethods() {
        ...
        while (methods.size() < METHOD_COUNT && guard++ < METHOD_COUNT * 50) {
            ...
            chosen.sort(Comparator.comparingInt(Bet365BttsOverUnderConsensusAnalyzer::familyRank)
    ...
    public Bet365BttsOverUnderConsensusAnalyzer() {
        try {
    ...
    /**
     * Havuz (pooled twin sətirləri) üzərində YALNIZ 4 marketin güvənini hesablayır:
     * Full Time -> KG VAR / KG YOX / 2.5 ÜST / 2.5 ALT. Başqa market hesablanmır.
     */
    private List<Pick> evaluatePicks(int[] pool) {
        List<Pick> picks = new ArrayList<>();

        // ── Full Time — Qarşılıqlı qol (BTTS) ──
        addPred(picks, pool, "FT KG VAR (BTTS Yes)", r -> rFtH[r] > 0 && rFtA[r] > 0);
        addPred(picks, pool, "FT KG YOX (BTTS No)",  r -> rFtH[r] == 0 || rFtA[r] == 0);

        // ── Full Time — 2.5 A/U ──
        addPred(picks, pool, "FT 2.5 ÜST (3+ qol)",  r -> ftTot(r) > 2);
        addPred(picks, pool, "FT 2.5 ALT (0-2 qol)", r -> ftTot(r) <= 2);

        return picks;
    }

    private void addPred(List<Pick> picks, int[] pool, String label, Predicate<Integer> hit) {
        int h = 0, t = 0;
        for (int r : pool) {
            if (!ftValid(r)) continue;
            t++;
            if (hit.test(r)) h++;
        }
    ...
    public void run() {
        ...
        System.out.println("═══════════════════════════════════════════════════════════════");
        System.out.printf("🤖 ORTAQ TƏXMİN — YALNIZ FT: KG VAR/YOX & 2.5 ÜST/ALT — %d oyun  (baza: %,d tarixi oyun)%n",
                today.size(), rowCount);
        System.out.println("═══════════════════════════════════════════════════════════════\n");
    ...
    private void printConsensus(MatchInfo match, Pick best, List<Pick> picks,
        ...
        for (int i = 1; i < picks.size(); i++) {
            ...
            System.out.printf("║    %2d) %-24s  %.1f%% (%d/%d)%n",
    ...
    public static void main(String[] args) {
        Bet365BttsOverUnderConsensusAnalyzer analyzer = new Bet365BttsOverUnderConsensusAnalyzer();
        analyzer.run();
